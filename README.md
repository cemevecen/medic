...
**Fiyatlar** sekmesinin dolması için `data/referans_bazli_ilac_fiyat_listesi.xlsx` ve/veya `data/ilac_fiyat_web_listesi.xlsx` dosyalarının mevcut olması gerekir (ayrıntı: `referans_ilac_fiyat.py`). İsteğe bağlı olarak `recete_haber` ile birleştirilen ek kaynaklar da kullanılır.
